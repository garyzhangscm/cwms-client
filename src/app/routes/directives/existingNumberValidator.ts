...
@Directive({
    selector: '[appExistingNumberValidator]',
    providers: [{ provide: NG_ASYNC_VALIDATORS, useExisting: ExistingNumberValidator, multi: true }],
    standalone: false
})
export class ExistingNumberValidator implements AsyncValidator {
  @Input() variable: string | undefined;

  constructor(private validatorServiceService: ValidatorServiceService) {
      console.log(`appExistingNumberValidator created for ${this.variable}`);
  }

  validate(ctrl: AbstractControl): Promise<ValidationErrors | null> | Observable<ValidationErrors | null> {
    console.log(`appExistingNumberValidator start validating ${this.variable} for value ${ctrl.value}`);
    if (ctrl.value === null) {
      return of(null);
    }
    // console.log(`start validating! ${this.variable} with value: ${ctrl.value}`);
    return this.validatorServiceService.validateExistingNumber(this.variable!, ctrl.value).pipe(
      map(errorCode => {
