...
export class WorkOrderBillOfMaterialMaintenanceComponent implements OnInit {
  ...
  billOfMaterialNumberOnBlur(billOfMaterialNumber: string): void {
    this.currentBillOfMaterial.number = billOfMaterialNumber;
  }
  itemNameOnBlur(itemName: string): void {
  ...
  byProductItemNameChanged(itemName: string, byProduct: BillOfMaterialByProduct): void {
    console.log(`Item of BOM change to ${itemName}`);
    if (itemName === '') {
