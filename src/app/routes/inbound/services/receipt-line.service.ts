...
export class ReceiptLineService {
  ...
  receiveInventory(receiptId: number, receiptLineId: number, inventory: Inventory, receiveToStage?: boolean): Observable<Inventory> {
    ...
    return this.http.post(`inbound/receipts/${receiptId}/lines/${receiptLineId}/receive`, inventory, params).pipe(map(res => res.data));
  }
  ...
  generatePrePrintLPNLabelInBatch(
    ...
    copies?: number,
    printerName?: string,
    ignoreInventoryQuantity?: boolean
  ...
  generatePrePrintLPNReportInBatch(
    receiptLineId: number,
    lpn: string,
    quantity?: number,
    count?: number,
