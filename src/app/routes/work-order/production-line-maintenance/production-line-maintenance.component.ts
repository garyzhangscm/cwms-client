...
export class WorkOrderProductionLineMaintenanceComponent implements OnInit {
  ...
  ngOnInit(): void {
    ...
    this.activatedRoute.queryParams.subscribe(params => {
      if (params['id']) {
        ...
        this.productionLineService.getProductionLine(params['id']).subscribe({
          next: productionLine => {
    ...
    this.unitOfMeasureService.loadUnitOfMeasures().subscribe(unitOfMeasureRes => (this.availableUnitOfMeasures = unitOfMeasureRes));

    // Load all moulds
    this.mouldService.getMoulds().subscribe(mouldsRes => (this.availableMoulds = mouldsRes));
  }
    ...
    this.warehouseConfigurationService.getWarehouseConfiguration().subscribe({
      next: warehouseConfiguration => {
        if (warehouseConfiguration.printingStrategy === PrintingStrategy.SERVER_PRINTER) {
          ...
          warehouseConfiguration.printingStrategy === PrintingStrategy.LOCAL_PRINTER_LOCAL_DATA ||
          ...
          this.printingService.getAllLocalPrinters().forEach((printer, index) => {
            this.availablePrinters.push({
              id: index,
              name: printer,
              description: printer,
  ...
  refreshListOfProductionLineCapacityItemData() {
    ...
    this.currentProductionLine.productionLineCapacities.forEach(productionLineCapacity => {
  ...
  confirm(): void {
    ...
    this.listOfProductionLineCapacityItemData.forEach(productionLineCapacityItemData => {
      this.currentProductionLine.productionLineCapacities.push(productionLineCapacityItemData.productionLineCapacity);
    });
    this.isSpinning = true;
    if (this.newProductionLine) {
      this.productionLineService.addProductionLine(this.currentProductionLine).subscribe(productionLine => {
      ...
      this.productionLineService.changeProductionLine(this.currentProductionLine).subscribe(productionLine => {
        this.messageService.success(this.i18n.fanyi('message.save.complete'));
        setTimeout(() => {
          this.isSpinning = false;
          this.router.navigateByUrl(`/work-order/production-line?name=${this.currentProductionLine.name}`);
  ...
  deleteProductionLineCapacityItemData(id: string): void {
    this.listOfProductionLineCapacityItemData = this.listOfProductionLineCapacityItemData.filter(d => d.id !== id);
  }
  ...
  mouldChanged(mouldName: string, productionLineCapacity: ProductionLineCapacity): void {
    this.mouldService.getMoulds(mouldName).subscribe(moulds => {
      if (moulds.length == 1) {
